/**
 * Phase 5 macOS backend security audit.
 * Native application lifecycle only — no shell, scripting, force-kill, UI automation.
 */
...
export interface MacOSBackendAuditReport {
  ok: boolean;
  scannedFiles: number;
  failures: string[];
  notes: string[];
...
export async function runMacOSBackendAudit(): Promise<MacOSBackendAuditReport> {
  const files = [
    ...(await walkTs(path.join(ROOT, "src/platform"))),
    ...(await walkTs(path.join(ROOT, "src/applications"))),
  ];
  ...
  notes.push("Phase 5: ApplicationBackend + MacOSApplicationBackend.");
  notes.push("Without N-API bridge, open/close/active remain UNAVAILABLE (honest).");
  notes.push("No shell, scripting bridge, force-kill, or UI automation.");
...
if (isDirect) {
  runMacOSBackendAudit().then((report) => {
    console.log(`macOS backend audit — scanned ${report.scannedFiles} source files`);
    for (const n of report.notes) console.log(`  note: ${n}`);
    if (report.ok) {
      console.log("macOS backend audit PASSED.");
    } else {
      console.error("macOS backend audit FAILED:");
