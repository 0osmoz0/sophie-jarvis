/**
 * Phase 15 security monitor audit — monitoring must stay action-free.
 * Forbids setInterval; recursive setTimeout scheduling is allowed.
 */
...
export interface SecurityMonitorAuditReport {
  ok: boolean;
  scannedFiles: number;
  failures: string[];
  notes: string[];
...
export async function runSecurityMonitorAudit(): Promise<SecurityMonitorAuditReport> {
  const files = [
  ...
  notes.push("Phase 15: SecurityMonitor = MONITORING + ALERT only.");
  notes.push("No ActionExecutor path from monitor → alert.");
  notes.push("setInterval forbidden; controlled setTimeout scheduling allowed.");
  notes.push("No shell, camera, mic, fetch, FileService, ApplicationService imports.");
...
if (isDirect) {
  runSecurityMonitorAudit().then((report) => {
    console.log(`Security monitor audit — scanned ${report.scannedFiles} files`);
    for (const n of report.notes) console.log(`  note: ${n}`);
    if (report.ok) console.log("Security monitor audit PASSED.");
    else {
      console.error("Security monitor audit FAILED:");
